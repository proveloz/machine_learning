...
def distplot_num(dataframe,lista,target):
    ...
    for i,n in enumerate(var):
        if n!=target:
            plt.subplot(4,2,i+1)
            sns.distplot(dataframe[n])
            plt.title(target+ "vs" +n,fontsize=15)
            sns.set_palette("bright")        
...
def predict_target(df_dummy,var):
    ...
    return y_modelo1_pred
#=modelo1_entrenado.predict(X_test),mean_squared_error(y_test,y_modelo1_pred),r2_score(y_test,y_modelo1_pred)
